...
The second line contains N space-separated integers (a1, a2, a3, ...., aN) describing the arrival times for each student.
...
For the first test case, K=3 . The professor wants at least 3 students in attendance, but only 2 have arrived on time
(-3 and -1). Thus, the class is canceled.

For the second test case, K=2. The professor wants at least 2 students in attendance, and there are 2 who have arrived
on time (0 and -1). Thus, the class is not cancelled.
"""
...
for a0 in xrange(t):
    ...
    students = filter(lambda x: x <= 0, a)
    if len(students) >= k:
        print "NO"
    else:
        print "YES"